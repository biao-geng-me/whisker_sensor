...
class Reader:
    ...
    def _compute_v0(self) -> np.ndarray:
        """Read ~Fs/4 samples, replace outliers, average per channel."""
        n = max(1, int(round(self._fs / 4.0)))
        buf = self._read_n_samples(n)
        cleaned = fill_outliers_linear(buf)
        return cleaned.mean(axis=0).astype(np.float32)

    # ----- frame reading

    def _read_n_samples(self, n: int) -> np.ndarray:
        """Block-read ``n`` complete frames and parse them. Used at startup."""
        total = n * frame_size_for(self._nch)
        raw = self._transport.read_exact(total)
    ...
    def _stream_loop(self) -> None:
        ...
        frame_total = ns_read * frame_size_for(self._nch)
        while not self._stop.is_set():
