# Hardware Handoff: v2 path7 policy (path-blind)

This package is a compact deployment handoff for the v2 path-blind SAC policy trained in:
- `rl_sac_v2_pathblind_runs/v2-pathblind-path7-48h-6env-pstart_47676053`

Selected checkpoint in this handoff:
- `checkpoint_0075000.pt`
- actor-only copy: `actor_only_0075000.pt`

Important note:
- Path 7 is only the training source; the deployed controller expects the same observation/kinematics format but no path geometry.
- For convenience, this package includes exported path7 geometry files (`path7_xy.csv` / `path7_xy.npy`). The policy does not use them.

## Package contents

- `checkpoint_0075000.pt`: original full SAC checkpoint from the chosen run
- `actor_only_0075000.pt`: actor weights only, for easier deployment
...
- `deploy_v2_path7.py`: minimal inference wrapper for hardware integration
- `simulate_v2_path7.py`: deterministic simulator rollout entry point using the handoff package
- `path7_xy.npy` / `path7_xy.csv`: exported path7 geometry for reference/plotting (not used by the policy)
...
```bash
python deploy_v2_path7.py --package-dir . --device cpu
```

In code, the intended usage is:
1. create `V2Path7HardwareRunner`
...
```python
runner = V2Path7HardwareRunner(package_dir=".")
...
```bash
python simulate_v2_path7.py --package-dir . --device cpu
```

By default it uses `actor_only_0075000.pt`, rebuilds the simulator config from `source_config.json`, runs a deterministic rollout, and writes results to a folder such as `sim_eval_actor_only_0075000/`.

Useful options:
- `--weights checkpoint_0075000.pt` to test the full checkpoint instead of the actor-only export
- `--path-sub path7` to force a specific path
